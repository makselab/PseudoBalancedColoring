...
indices = foreach(i = 1:length(files), .combine = rbind) %do% {
  ...
  index = rbind(index, c("Number of Non-Trivial Colors", ncolors - sum(table(balancedColoring$Color) == 1)))
...
# # variables = c("Number of Trivial Colors", "Number of Non-Trivial Colors", "Number of Nodes in Non-Trivial Colors", "Normalized Fiedler Value", "Mean epsilon")
...
# manualIndices = data.frame(V1 = read.table("manualRepairIndices", sep = "\n", stringsAsFactors = F)[c(16:18, 23), ], stringsAsFactors = F)
